...
export const Badge = ({ children, variant = "default", className = "" }: BadgeProps) => {
    const variants: Record<string, string> = {
        ...
        guest: "border-transparent bg-orange-100 text-orange-800",
        success: "border-transparent bg-emerald-100 text-emerald-800",
        tech: "border-transparent bg-blue-50 text-blue-700 border border-blue-100",
    };
    return (
        <div
...
export const Alert = ({ icon: Icon, title, children, variant = "default" }: AlertProps) => {
    ...
    return (
        <div
            className={`relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-slate-950 ${styles[variant]}`}
        >
...
export const Kbd = ({ children }: KbdProps) => (
    <kbd className="pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border border-slate-200 bg-slate-100 px-1.5 font-mono text-[10px] font-medium text-slate-500 shadow-[0px_1px_0px_0px_#e2e8f0]">
        {children}
    </kbd>
);
...
export const CodeBlock = ({ children }: CodeBlockProps) => (
    <code className="relative rounded bg-slate-100 px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold text-slate-900">
        {children}
    </code>
);
